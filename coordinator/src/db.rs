...
create_db!(
  MainDb {
    ...
    FirstPreprocessDb: (network: NetworkId, id_type: RecognizedIdType, id: [u8; 32]) -> Vec<u8>,
...
impl FirstPreprocessDb {
  pub fn save_first_preprocess(
    ...
    let key = Self::key(network, id_type, id);
...
impl ExpectedBatchDb {
  pub fn save_expected_batch(txn: &mut impl DbTxn, batch: &Batch) {
    ...
    Self::set(txn, batch.network, batch.id, &data);
  }
}
