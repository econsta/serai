...
impl<D: Db, T: TransactionTrait> Mempool<D, T> {
  ...
  fn save_tx(&mut self, tx: Transaction<T>) {
    ...
    let mut current_mempool = CurrentMempoolDb::get(&self.db, self.genesis).unwrap_or(vec![]);
  ...
  pub(crate) fn new(db: D, genesis: [u8; 32]) -> Self {
    ...
    let current_mempool = CurrentMempoolDb::get(&res.db, res.genesis).unwrap_or(vec![]);
  ...
  pub(crate) fn remove(&mut self, tx: &[u8; 32]) {
    ...
    if i != current_mempool.len() {
      CurrentMempoolDb::set(&mut txn, self.genesis, &[&current_mempool[.. i], &current_mempool[(i + 32) ..]].concat());
    }
    txn.commit();
